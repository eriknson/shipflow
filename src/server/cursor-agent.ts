...
const buildCandidateDirs = (
  ...
  if (HOME_DIR) {
    ...
    candidateDirs.add(
      path.join(HOME_DIR, 'Library', 'Application Support', 'Cursor', 'bin'),
    );
    candidateDirs.add(
      path.join(HOME_DIR, 'AppData', 'Local', 'Programs', 'cursor', 'bin'),
    );
  }
...
async function discoverCursorAgentBinary(
    ...
    try {
      ...
      const timeoutMs =
        typeof options.timeoutMs === 'number'
          ? options.timeoutMs
          : Number(
              process.env.SHIPFLOW_OVERLAY_AGENT_TIMEOUT_MS ?? 4 * 60 * 1000,
            );
      ...
      const processLine = (line: string) => {
        ...
        try {
          ...
          if (typeof parsed.type === 'string' && parsed.type === 'error') {
            ...
            if (
              errorMessage.toLowerCase().includes('cannot use this model') ||
          ...
          console.warn(
            `${logPrefix} Failed to parse cursor-agent stream line`,
            {
              line,
              error,
            },
      ...
      const timeoutId = setTimeout(() => {
        ...
        console.warn(
          `${logPrefix} cursor-agent exceeded timeout; terminating process`,
          {
            timeoutMs,
          },
        );
        ...
        sendStatus(
          `Cursor CLI timed out after ${timeoutMs}ms; terminating process.`,
        );
        ...
        try {
          ...
          console.warn(
            `${logPrefix} Failed to terminate cursor-agent process`,
            killError,
          );
        }
      ...
      child.stderr.on('data', (chunk) => {
        ...
        if (
          (lowerText.includes('cannot use this model') ||
      ...
      console.error(
        `${logPrefix} Unexpected error launching cursor-agent`,
        error,
      );
      send({
        event: 'done',
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Unexpected error launching Cursor CLI.',
      });
      resolve();
    }
  });
