...
const extractComponentNames = (stackTrace: string | null): string[] => {
  ...
  const matches = stackTrace.matchAll(
    /\b(?:at|in)\s+([A-Z][a-zA-Z0-9]*)\s*\(Server\)/g,
  );
...
const buildPrompt = (
  ...
  if (filePath) {
    ...
    lines.push(
      'Find the file containing the component that renders this HTML:',
    );
    lines.push(htmlFrame ?? '(no HTML frame provided)');
    lines.push('');
    ...
    if (componentNames.length > 0) {
      lines.push('');
      lines.push(
        `Look for files defining these components: ${componentNames.join(', ')}`,
      );
    }
  }
...
export function createNextHandler(options: ShipflowOverlayServerOptions = {}) {
  ...
  return async function handler(request: NextRequest) {
    ...
    const model =
      payload.model?.trim() || options.defaultModel || DEFAULT_MODEL;
    ...
    try {
      ...
      const { resolveCursorAgentBinary, runCursorAgentStream } = await import(
        './cursor-agent'
      );
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          try {
            ...
            console.error(
              `${logPrefix} Failed during Cursor CLI streaming`,
              error,
            );
            send({
              event: 'done',
